...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        for j in range(num_test):
            dists[i, j] = torch.sum((flatten_train[i] - flatten_test[j]) ** 2)
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        diff = flatten_train[i] - flatten_test
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # M - train N - test
    MM = torch.sum(flatten_train ** 2, dim=1, keepdim=True)
    NN = torch.sum(flatten_test ** 2, dim=1).view(1, -1)
    MN = torch.mm(flatten_train, flatten_test.t())
    dists = MM -2 * MN + NN
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    _, top_k_indices = torch.topk(dists, k, dim=0, largest=False, sorted=True)
    neighbor_labels = y_train[top_k_indices]
    # neighbor_labels.shape == (k, num_test)
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            train_data = torch.cat(x_train_folds[0:i] + x_train_folds[i + 1:])
            train_label = torch.cat(y_train_folds[0:i] + y_train_folds[i + 1:])
            test_data = x_train_folds[i]
            test_label = y_train_folds[i]
            classifier = KnnClassifier(train_data, train_label)
            acc.append(classifier.check_accuracy(test_data, test_label, k, quiet= True))
