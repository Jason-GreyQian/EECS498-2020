...
def mutate_tensor(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    assert len(indices) == len(values), "length must be equal"
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x = torch.tensor([x for x in range(start, stop) if x % 10 == 0], dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    # y = x.clone()
    # min_val, min_pos = x.min(dim=1)
    # row_idx = torch.arange(x.shape[0])
    # y[row_idx, min_pos] = 0
    y = x.clone()
    y[torch.arange(x.size(0)), x.argmin(dim=1)] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    x_mean = torch.sum(x, dim=0, keepdim=True) / M
    x_std = (torch.sum((x - x_mean) ** 2, dim=0, keepdim=True) / (M - 1)) ** 0.5
    y = (x - x_mean) / x_std 
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    indices = torch.repeat_interleave(
        torch.arange(len(ls), device=ls.device),
        ls
    )
    
    # 分段求和
    sums = torch.zeros(len(ls), dtype=all_values.dtype, device=all_values.device)
    sums.index_add_(0, indices, all_values)
    
    # 计算均值（转换为相同数据类型）
    return sums / ls.to(dtype=sums.dtype)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
